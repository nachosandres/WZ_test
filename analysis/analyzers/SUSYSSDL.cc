...
bool SUSYSSDL::electronSelection(int elIdx){
  ...
  counter("ElectronDenominator", kElId);
	...
	if(!makeCut<int>(       _vc -> getI(_lep + "_tightId", elIdx) ,  1     , "=", "POG MVA Tight Id ", 0    , kElId)) return false;
...
bool SUSYSSDL::vetoEventSelection(std::string electron_label, std::string muon_label, std::string kr){
	...
	if(_NumKinObj[muon_label] >= 1 && _NumKinObj["VetoMuon"] == 1){
		...
		if(os){
			...
			if(makeCut(_vc -> getF(_lep + "_pt", _KinObj["VetoMuon"][0]) > _Cuts["VetoMuonPtLow"]  && mll < _Cuts[kr + "VetoMLLLow"], "muon mll low pt veto", "=", kVetoLepSel) ) return true;

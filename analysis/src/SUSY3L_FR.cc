...
SUSY3L_FR::SUSY3L_FR(std::string cfg){
    /* 
        constructs the SUSY3L_FR class 
        parameters: configuration_file
        return: none
    */
	
    //_verbose->Class("SUSY3L_FR");
    startExecution(cfg);
    initialize();
}
...
//____________________________________________________________________________
void SUSY3L_FR::initialize(){
    /*
        initializes the SUSY3L_FR class
        parameters: none
        return: none
    */
...
void SUSY3L_FR::defineOutput(){
    ...
    */
    ...
    _hm->addVariable("BR_FRel"      , 100 ,   0.0,   1.0, "FR electrons"                    ); 
    _hm->addVariable("BR_FRmu"      , 100 ,   0.0,   1.0, "FR muons"                        ); 
...
void SUSY3L_FR::writeOutput(){
    ...
    */
    ...
    _hm -> saveHistos("SUSY3L_FR", _cfgName);
    _au -> saveNumbers("SUSY3L_FR", _cfgName);

}


//____________________________________________________________________________
void SUSY3L_FR::modifySkimming(){
...
void SUSY3L_FR::collectKinematicObjects(){
...
//____________________________________________________________________________
bool SUSY3L_FR::electronSelection(int elIdx){
    /*
...
//____________________________________________________________________________
bool SUSY3L_FR::vetoElectronSelection(int elIdx){
    /*
        selection of electrons in sideband region
        parameters: elIdx
    ...
    */
    ...
    //define cuts for sideband electrons
    float pt_cut_low = 10.;
    float pt_cut_high = 35.;
    float eta_cut = 2.4;
    float eta_veto_low = 1.4442;
    float eta_veto_high = 1.566;
    float isolation_cut = 0.15;
    ...
    //apply the cuts
    //makeCut(variable to cut on, cut value, direction of acception, name, 2nd cut value, counter)
    if(!makeCut<float>( _vc->get("LepGood_pt", elIdx) , pt_cut_low, "[]"  , "pt selection"    , pt_cut_high    , kElVeto)) return false;
    if(!makeCut<float>( std::abs(_vc->get("LepGood_eta", elIdx)), eta_cut  , "<"  , "eta selection"   , 0    , kElVeto)) return false;
    if(!makeCut<float>( std::abs(_vc->get("LepGood_eta", elIdx)), eta_veto_low, "[!]", "eta selection veto"   , eta_veto_high, kElVeto)) return false;
    if(!makeCut<int>( _vc->get("LepGood_eleCutIdCSA14_50ns_v1", elIdx) , 3     , ">=" , "POG CB WP-M Id " , 0    , kElVeto)) return false;
    if(!makeCut<float>( _vc->get("LepGood_relIso03", elIdx) , isolation_cut   , ">"  , "non-isolation "      , 0    , kElVeto)) return false;
    if(!makeCut<float>( std::abs(_vc->get("LepGood_dz", elIdx)), vertex_dz_cut   , "<"  , "dz selection"    , 0    , kElVeto)) return false;
    if(!makeCut<float>( std::abs(_vc->get("LepGood_dxy", elIdx)), vertex_dxy_cut  , "<"  , "dxy selection"   , 0    , kElVeto)) return false;
    if(!makeCut<float>( std::abs(_vc->get("LepGood_sip3d", elIdx)), sip3d_cut  , ">"  , "sip3d selection"   , 0    , kElVeto)) return false;
    if(!makeCut<int>( _vc->get("LepGood_tightCharge", elIdx) , 1     , ">"  , "charge selection", 0    , kElVeto)) return false;
    //boolian variable if electron comes from gamme conversion or not (true if not from conversion)
    bool not_conv = (_vc->get("LepGood_convVeto", elIdx)>0 && _vc->get("LepGood_lostHits", elIdx)==0);
    if(!makeCut( not_conv, "conversion rejection", "=", kElVeto)) return false;
    ...
    if(!makeCut( !muMatch, "dR selection (mu)", "=", kElVeto) ) return false;
...
//____________________________________________________________________________
bool SUSY3L_FR::muonSelection(int muIdx){
    /*
        selection of muons
        parameters: muIdx
...
//____________________________________________________________________________
bool SUSY3L_FR::vetoMuonSelection(int muIdx){
    /*
    ...
    */
    ...
    //apply the cuts
    if(!makeCut<float>( _vc->get("LepGood_pt", muIdx), pt_cut_low, "[]", "pt selection"    , pt_cut_high, kMuVeto)) return false;
    if(!makeCut<float>( std::abs( _vc->get("LepGood_eta", muIdx)), eta_cut, "<", "eta selection", 0, kMuVeto)) return false;
    if(!makeCut<float>( _vc->get("LepGood_relIso03", muIdx) , isolation_cut   , ">", "non-isolation "      , 0, kMuVeto)) return false;
    if(!makeCut<int>( _vc->get("LepGood_tightId", muIdx) , 1     , "=", "POG Tight Id "   , 0, kMuVeto)) return false;
    if(!makeCut<float>(std::abs(_vc->get("LepGood_dz", muIdx)), vertex_dz_cut   , "<", "dz selection"    , 0, kMuVeto)) return false;
    if(!makeCut<float>(std::abs(_vc->get("LepGood_dxy", muIdx)), vertex_dxy_cut , "<", "dxy selection"   , 0, kMuVeto)) return false;
    if(!makeCut<float>( std::abs(_vc->get("LepGood_sip3d", muIdx)), sip3d_cut  , ">"  , "sip3d selection"   , 0    , kMuVeto)) return false;
...
//____________________________________________________________________________
bool SUSY3L_FR::tauSelection(int tauIdx){
    /*
        selection of taus
        parameters: tauIdx
...
//____________________________________________________________________________
bool SUSY3L_FR::goodJetSelection(int jetIdx){
    /*
        selection of jets
        parameters: jetIdx
...
//____________________________________________________________________________
bool SUSY3L_FR::bJetSelection(int jetIdx){
    /*
...
//____________________________________________________________________________
void SUSY3L_FR::setBaselineRegion(){
    /*
    ...
    */
    ...
    if(_BR == "BR0"){
        //setCut("LepMultiplicity"    ,    1, ">=" )  ;     //number of isolated leptons
        setCut("vetoLepMultiplicity",    1, "="  )  ;     //number of isolated leptons
        ...
        _lowMTCut                    = 15.         ;     //low transverse mass cut bjet and probe lepton
        //setCut("HT"                 ,   60, ">=" )  ;     //sum of jet pT's
        setCut("MET"                ,   15, "<"  )  ;     //missing transverse energy
    }
...
//____________________________________________________________________________
void SUSY3L_FR::setSignalRegion() {
...
//____________________________________________________________________________
void SUSY3L_FR::setCut(std::string var, float valCut, std::string cType, float upValCut) {
    /*
    ...
    */
    ...
    if(var == "vetoLepMultiplicity") {
        _valCutVLepMultiplicityBR   = valCut;
        _cTypeVLepMultiplicityBR    = cType;
        _upValCutVLepMultiplicityBR = upValCut;
    }
    else if(var == "NJets") {
...
bool SUSY3L_FR::baseSelection(){
    ...
    */
    ...
    //require minimum number of veto leptons
    if(!makeCut<int>( _nVEls + _nVMus, _valCutVLepMultiplicityBR, _cTypeVLepMultiplicityBR, "veto lepton multiplicity", _upValCutVLepMultiplicityBR ) ) return false;

    //require prompt lepton back to back with b-jet
    bool has_back_lepton = back_lepton();
    if(!makeCut( has_back_lepton , "has probe lepton opposite to bjet", "=") ) return false;
    ...
    //MT requirement (bjet and probe lepton)
    bool mt_fulfilled = small_mt_bjet_lepton();
    if(!makeCut( mt_fulfilled , "MT bjet - probe lepton requirement", "=") ) return false;

    //missing transvers energy requirement (actually missing momentum)
...
//____________________________________________________________________________
bool SUSY3L_FR::hardLegSelection(){
    /*
...
//____________________________________________________________________________
bool SUSY3L_FR::back_lepton(){
    /*
        Checks if the event has a veto lepton in the opposite direction to the bjet
        return: true (if the event has such a lepton), false (else)
    */
    
    float deltaPhi_cut = 2;

    //check if the veto electron (if there) is back to back with all bjets
    if(_nVEls == 1){
        for(int ibjet=0; ibjet<_nBJets; ++ibjet){
            float deltaPhi = DeltaPhi(_vEls[0]->phi(), _bJets[ibjet]->phi());
            if(deltaPhi < deltaPhi_cut){
                return false;
            }
        }
        return true;
    ...
    //check if the veto muon (if there) is back to back with all bjets
    else if(_nVMus == 1){
        for(int ibjet=0; ibjet<_nBJets; ++ibjet){
            float deltaPhi = DeltaPhi(_vMus[0]->phi(), _bJets[ibjet]->phi());
            if(deltaPhi < deltaPhi_cut){
                 return false;
            }
        }
...
//____________________________________________________________________________
bool SUSY3L_FR::small_mt_bjet_lepton(){
    /*
        Checks if the transverse mass of any bjet in the event with the probe lepton 
        if smaller than the cut value
        return: true (if MT is small enough), false (else)
    */
    float mt = 0.;
    
    //compute transverse mass of lepton and any bjet
    if(_nVEls == 1){
        for(int ibjet=0; ibjet<_nBJets; ++ibjet){
            mt = M_T(_vEls[0]->pt(), _bJets[ibjet]->pt(), _vEls[0]->phi(), _bJets[ibjet]->phi());
            if(mt > _lowMTCut){
                return false;
            }
        }
    ...
    if(_nVMus == 1){
        for(int ibjet=0; ibjet<_nBJets; ++ibjet){
            mt = M_T(_vMus[0]->pt(), _bJets[ibjet]->pt(), _vMus[0]->phi(), _bJets[ibjet]->phi());
            if(mt > _lowMTCut){
                return false;
            }
        }
...
//____________________________________________________________________________
//bool SUSY3L_FR::ZEventSelection(){
    /*
...
//____________________________________________________________________________
bool SUSY3L_FR::ZEventSelectionLoop(){
    /*
...
//____________________________________________________________________________
void SUSY3L_FR::fillEventPlots(std::string kr){
    /*
...
//____________________________________________________________________________
float SUSY3L_FR::DeltaPhi(float phi1, float phi2){
    /*
...
//____________________________________________________________________________
float SUSY3L_FR::M_T(float pt_lepton, float met, float phi_lepton, float phi_met){
    /*

...
#for is in ${SAMPLES[@]}; do
    ...
    for sr in ${SRS[@]}; do
		...
		ib=${BRS[$ii]}
		cp cfg/templateSUSY.cfg cfg/tmpFiles/susy_${sr}_${pt}.cfg
		sed -i 's|SIGNALREGION|'$sr'|' cfg/tmpFiles/susy_${sr}_${pt}.cfg
		sed -i 's|BASELINEREGION|'$ib'|' cfg/tmpFiles/susy_${sr}_${pt}.cfg
		sed -i 's|PTSELECTION|'$pt'|' cfg/tmpFiles/susy_${sr}_${pt}.cfg
